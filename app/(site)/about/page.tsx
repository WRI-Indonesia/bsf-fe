...
export default async function About() {
  ...
  const heroSection = aboutContent?.hero_section as
    | Record<string, unknown>
    | undefined;
  const heroLabel = getSectionField(heroSection, "label", "ABOUT THE FORUM");
  const heroTitle = getSectionField(
    heroSection,
    "title",
    "Advancing biodiversity science through regional collaboration",
  );
  const heroDescription = getSectionField(
    heroSection,
    "description",
  ...
  const missionSection = aboutContent?.mission_section as
    | Record<string, unknown>
    | undefined;
  const missionLabel = getSectionField(
    missionSection,
    "label",
    "MISSION & OBJECTIVES",
  );
  const missionTitle = getSectionField(
    missionSection,
    "title",
    "Bridging science and policy for biodiversity action",
  );
  const missionDescription = getSectionField(missionSection, "description", "");
  const missionReadMore = getSectionField(
    missionSection,
    "read_more_text",
    "Read full mission statement",
  );
  const missionReadMoreUrl = getSectionField(
    missionSection,
    "read_more_url",
    "#",
  );
  ...
  const milestonesSection = aboutContent?.milestones_section as
    | Record<string, unknown>
    | undefined;
  const milestonesLabel = getSectionField(
    milestonesSection,
    "label",
    "OUR JOURNEY",
  );
  const milestonesTitle = getSectionField(
    milestonesSection,
    "title",
    "Key Milestones",
  );
  const milestones = getSectionArray<Milestone>(
    milestonesSection,
    "milestones",
  );

  const expertsSection = aboutContent?.experts_section as
    | Record<string, unknown>
    | undefined;
  const expertsLabel = getSectionField(
    expertsSection,
    "label",
    "SCIENTIFIC COMMITTEE",
  );
  const expertsTitle = getSectionField(
    expertsSection,
    "title",
    "Meet our Experts",
  );
        ...
        <section className="bg-background-base-lime-light mt-[98px] py-30 overflow-hidden">
          <div className="flex flex-col xl:flex-row px-10 md:px-20 items-stretch gap-30">
            <div className="flex flex-col z-10 gap-12 w-full xl:w-[703px] xl:h-[552px] shrink-0 justify-center">
              ...
              <p className="font-['inter'] text-xl text-text-grey-dark leading-relaxed xl:leading-1.6">
                {heroDescription}
              </p>
            </div>
        ...
        <section className="bg-text-white-broken px-20 py-30">
          <div className="mx-auto max-w-[1400px] grid lg:grid-cols-2 gap-16 lg:gap-24">
            <div className="flex flex-col gap-6">
              ...
              <a
                href={missionReadMoreUrl}
                className="inline-flex items-center gap-2 font-['inter'] text-[15px] font-semibold text-text-green hover:underline"
              >
            ...
            <div className="flex flex-col gap-8 md:max-w-[503px]">
              {objectives.map((item, i) => (
                <div key={item.id || i} className="flex gap-4">
                  ...
                  <div>
                    <h3 className="font-['inter'] text-xl font-semibold text-text-black">
                      {item.title}
                    </h3>
                    <p className="font-['inter'] text-base text-text-grey-dark">
                      {item.description}
                    </p>
                  </div>
                </div>
          ...
          <div className="mx-auto max-w-[1000px]">
            ...
            <div className="relative">
              ...
              {milestones.map((milestone, i) => (
                <div
                  key={milestone.id || i}
                  className={`relative flex items-center mb-16 last:mb-0 ${milestone.align_right ? "justify-end" : "justify-start"}`}
                >
                  <div
                    className={`w-1/2 ${milestone.align_right ? "pl-12" : "pr-12 text-right"}`}
                  >
                    <p className="font-['inter'] text-sm font-semibold text-text-green mb-1">
                      {milestone.year}
                    </p>
                    <h4 className="font-['inter'] text-lg font-semibold text-text-green mb-2">
                      {milestone.title}
                    </h4>
                    {milestone.description && (
                      <p className="font-['inter'] text-base text-text-green">
                        {milestone.description}
                      </p>
                    )}
                  </div>
          ...
          <div className="flex flex-col mx-auto max-w-[1400px] gap-8 md:gap-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-10">
              {experts.map((expert, i) => {
                const expertImg = getUploadUrl(
                  expert as unknown as Record<string, unknown>,
                  "image",
                  "/about/expert_1.png",
                );
                const socialLinks = expert.social_links as
                  | SocialLinks
                  | undefined;
                ...
                return (
                  <div
                    key={expert.id || i}
                    className="flex flex-col xl:flex-row gap-5 rounded-2xl p-4 sm:p-5 items-center xl:items-start text-center xl:text-left"
                  >
                    ...
                    <div className="flex flex-col justify-center h-full p-2 xl:p-0 w-full min-w-0">
                      <h3 className="text-[24px] font-bold text-text-black">
                        {expert.name}
                      </h3>
                      <p className="font-['inter'] text-[14px] font-semibold text-[#0A77FF] mb-4">
                        {expert.role}
                      </p>
                      ...
                      <div className="flex gap-4 items-center justify-center xl:justify-start">
                        {(
                          ["x", "facebook", "linkedin", "telegram"] as const
                        ).map((platform) => {
