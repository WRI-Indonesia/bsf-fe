...
export default async function Events() {
  ...
  const heroEvent = heroSection?.featured_event as
    | Record<string, unknown>
    | undefined;
  const heroTitle =
    (heroEvent?.title as string) || "4th Biodiversity Science Forum 2026";
  const heroDate =
    formatDateRange(
  ...
  const heroParticipants =
    (heroEvent?.participants as string) || "500+ Expected Participants";
  const heroDescription =
    (heroEvent?.description as string) ||
  ...
  const heroImage = getUploadUrl(
    heroSection,
    "image",
    ((heroEvent?.image as Record<string, unknown>)?.url as string) ||
      "/events/hero.png",
  );
            ...
            >
              ...
              <div className="flex w-full flex-col items-center justify-end gap-3">
                ...
                <p className="w-full font-['inter'] text-[14px] leading-none text-text-grey-light">
                  note : Registration is by invitation only, selected authors
                  will receive an email link after abstract review.
                </p>
              </div>
            </div>
                    ...
                    >
                      ...
                      <div className="flex flex-col justify-center h-full gap-8">
                        ...
                        <div className="flex gap-4 items-center">
                          {(
                            ["x", "linkedin", "facebook", "telegram"] as const
                          ).map((platform) => {
